...
constructor fbext_Vector4((T_)) ( byval x as fbext_TypeName( T_), byval y as fbext_TypeName( T_), byval z as fbext_TypeName( T_), byval w as double )
...
constructor fbext_Vector4((T_)) ( byref v4d as fbext_Vector4((T_)) )
...
function fbext_Vector4((T_)).dot ( byref v As fbext_Vector4((T_)) ) as fbext_TypeName( T_)
    Return  this.x * v.x + this.y * v.y + this.z * v.z + this.w * v.w
end function

function fbext_Vector4((T_)).magnitude( ) As fbext_TypeName( T_)
...
sub fbext_Vector4((T_)).normalize()
    this = this / this.magnitude()
end sub

function fbext_Vector4((T_)).cross( byref v as fbext_Vector4((T_)) ) as fbext_Vector4((T_))
    return Type<fbext_Vector4((T_))>((this.y * v.z) - (v.y * this.z), (this.z * v.x) - (v.z * this.x), (this.x * v.y) - (v.x * this.y), this.w)
end function

function fbext_Vector4((T_)).distance( byref v as fbext_Vector4((T_)) ) as fbext_TypeName( T_)
    return Sqr((v.x - this.x)^2 + (v.y - this.y)^2 + (v.z - this.z)^2)
end function

operator fbext_Vector4((T_)).cast() as string
    return "x: " & this.x & ", y: " & this.y & ", z: " & this.z & ", w: " & this.w
end operator

operator fbext_Vector4((T_)).cast() as fbext_Vector3((T_))
    return type<fbext_Vector3((T_))>(this.x, this.y, this.z)
end operator

operator + ( byref lhs as fbext_Vector4((T_)), byref rhs as fbext_Vector4((T_)) ) as fbext_Vector4((T_))
    return type<fbext_Vector4((T_))>( lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z, lhs.w + rhs.w )
end operator

operator + ( byref lhs as fbext_Vector4((T_)), byref rhs as fbext_TypeName( T_) ) as fbext_Vector4((T_))

...
type Vector as ext.math.fbext_Vector3( ((double)) )
    ...
    private sub Test
        ...
        TESTLY_ASSERT_TRUE( FBEXT_FLOAT_EQUAL( 5.0, f.Magnitude() ) )
        
        var g = Vector( 3.0, 4.0, 0.0 )
        TESTLY_ASSERT_TRUE( FBEXT_FLOAT_EQUAL( 5.0, g.Magnitude() ) )
        
        var h = Vector( 0.0, 3.0, 4.0 )
        TESTLY_ASSERT_TRUE( FBEXT_FLOAT_EQUAL( 5.0, h.Magnitude() ) )
    
    end sub
    
	private sub register constructor
		ext.testly.addSuite("ext-math-vector3-magnitude")
